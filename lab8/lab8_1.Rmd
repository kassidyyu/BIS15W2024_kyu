...
```{r}
penguins %>% # can save this as its own object for plotting later
  group_by(island, species) %>% # island and then species, can switch
...
penguins %>%
  ...
  summarize(mean_bill_length = mean(bill_length_mm, na.rm=T),
            ...
            n = n())
# this is very long and prone to mistakes
```
...
penguins %>%
  group_by(species) %>%
  summarize(mean_bill_legnth = mean(bill_length_mm, na.rm=T))
```
...
penguins %>%
  summarize(across(!c(species, island, sex, year), 
                   mean, na.rm=T)) # more efficient ways to do the other things we know how to do
```
