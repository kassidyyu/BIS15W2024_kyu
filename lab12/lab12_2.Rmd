...
```{r}
q <- homerange %>%
  ggplot(aes(x = log10.mass, y = log10.preymass, color = taxon))+ #not fill, color for scatter
  geom_point(na.rm=T)
```
...
```{r}
p+theme_linedraw()+
  theme(legend.position = "left", #move legend
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL, #removes label of x-axis - already have names of taxa
       y= "n",
       fill= "Trophic Guild")
```
...
```{r}
my_palette <- paletteer_d("miscpalettes::earthTones") #store your palette
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_wrap(~trophic.guild)+ #split into 2
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_grid(trophic.guild~.)+ # same but in rows instead of columns - can do ".~" first for side-by-side
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_grid(trophic.guild~thermoregulation)+ # 4 sections all at once
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
