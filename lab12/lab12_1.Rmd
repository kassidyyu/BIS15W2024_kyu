...
deserts %>%
  ...
  pivot_wider(names_from = species_id, values_from = n) # nice table - year - counts under DM, DS
```

```{r}
deserts %>%
  filter(species_id == "DM" | species_id == "DS") %>%
  mutate(year=as.factor(year)) %>% #makes gaps if remove this line
  group_by(year, species_id) %>%
  summarize(n = n(), .groups = 'keep') %>%
  ...
  labs(title = "Number of Samples for Species DM & DS", x = "Year", y = "n", color = "Species")
```
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "wheat4", fill = "lavender", bins=10)+ #color = outline, fill = fill color
  labs(title = "Distribution of Body Mass")
```
...
```{r}
homerange %>% 
  ggplot(aes(x = log10.mass)) + #only need to specify x
...
homerange %>% 
  ggplot(aes(x=log10.mass)) +
  geom_histogram(aes(y = after_stat(density)), fill = "deepskyblue4", alpha = 0.4, color = "black")+ #overlay
...
homerange %>%
  ...
  geom_histogram(color = "orchid4", fill = "azure2")+
...
homerange %>%
  ggplot(aes(x = log10.hra))+
  geom_density(color = "indianred", fill = "linen", alpha = 0.3)
```
...
homerange %>%
  ggplot(aes(x = log10.hra))+
  geom_histogram(aes(y = after_stat(density)), fill = "violetred4", alpha = 0.3, color = "grey10")+
  geom_density(color = "palevioletred")+
  labs(title = "Distribution of Homerange")
#consistency! same color, same fill throughout
```
...
homerange %>%
  mutate(mass_category = case_when(log10.mass<= 1.75 ~ "small",
                                   ...
                                   log10.mass>2.75 ~ "large")) # change continuous to categorical
```
