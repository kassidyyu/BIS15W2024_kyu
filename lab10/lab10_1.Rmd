...
#can be easily drawn on a piece of paper
```
...
```{r}
ggplot(data = iris,
       mapping = aes(x=Sepal.Width, y=Sepal.Length))+ #plus! not pipe
  geom_point()
```
...
# log transform since there can be a huge difference between very small or large animals
```
...
#sometimes overplotting with big data - mushed together, tries to fit
```
...
ggplot(data = homerange, mapping = aes(x=log10.hra, y=log10.preymass))+
  geom_point(na.rm=T)+
  geom_smooth(method = lm, se=T, na.rm=T) #linear model, standard error
#leaves out NAs
...
```{r}
homerange %>% #can use pipes instead of `data=`
  ggplot(aes(trophic.guild)) + #don't need `x=`
...
homerange %>% 
  ...
  ggplot(aes(y=common.name, x=log10.mass))+ #notice the switch in x and y - specify `y` first to flip
  geom_col()
```
...
homerange %>%
  filter(taxon == "mammals") %>%
  count(trophic.guild) %>% #math
  ggplot(aes(x=trophic.guild, y=n))+
  geom_col() #plot
#geom_col - categorical & continuous, x and y
#geom_bar - only need an x, bar for counts
```
