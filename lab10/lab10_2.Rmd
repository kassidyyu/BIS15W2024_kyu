...
penguins %>%
  ...
  ggplot(aes(species))+
  geom_bar()
#same, just other one generates count first
```
...
```{r}
penguins %>%
  filter(sex != "NA") %>% #don't need to do but cleans up a bit
  group_by(sex) %>%
  summarize(mean_bill_length = mean(bill_length_mm)) %>%
  ggplot(aes(x=sex, y=mean_bill_length))+
  geom_col()
#bigger code chunks = be careful! don't make new objects every chunk
```
...
#takes more coding to get a chart
```

```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass)) + #no need to specify group_by
  geom_boxplot()
#helpful ways to represent things with a range of values
```
...
```{r}
homerange %>%
  group_by(trophic.guild) %>% # helps to have this before plotting
  summarize(min_log10.mass=min(log10.mass),
            max_log10.mass=max(log10.mass),
            median_log10.mass=median(log10.mass))
```
