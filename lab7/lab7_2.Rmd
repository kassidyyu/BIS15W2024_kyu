...
penguins %>%
  group_by(island) %>%
  summarize(number_miss_bodymass = sum(is.na(body_mass_g)))
```

Well, that won't work so let's remove the NAs and recalculate.
```{r}
penguins %>%
  filter(!is.na(body_mass_g)) %>% #pull out all of the observations with a number, quicker than na.rm=T on every one
...
penguins %>%
  ...
  group_by(species) %>%
  summarize(mean_bill_length = mean(bill_length_mm), #could also put na.rm=T in side mean()
            n=n())
```
...
```{r}
penguins %>%
  summarize(across(c(species, island, sex), n_distinct)) #cleans up, variables of interest in c(), function after
```

This is very helpful for continuous variables.
```{r}
penguins %>%
  summarize(across(contains("mm"), mean, na.rm=T)) #variables that contain "mm" in the name, goes across mean, spits out
```

```{r}
penguins %>%
  summarize(across(contains("mm"), \(x) mean(x, na.rm = TRUE))) # \(x) will correct the error message
```
