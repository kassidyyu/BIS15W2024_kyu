...
```{r}
large %>%
  summarize(mean_large_sleep = mean(sleep_total))
```
...
msleep %>%
  ...
  summarize(min_bodywt = min(bodywt), #creates a column 'min_bodywt' for each unique 'vore'
...
```{r}
# NA means that a mean could not be calculated, at least one brainwt is NA
msleep %>%
...
#Cetacea have a 'NaN' mean brainwt, it is not a number
```
...
msleep %>%
  ...
  select(order, genus, brainwt)
#we can see that there are no brainwt measurements for Cetacea (whales)
```
