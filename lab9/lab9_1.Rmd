...
The column names are observations
...
relig_income %>%
  pivot_longer(-religion,
               names_to = "income",
               values_to = "total")
# each column is its own variable, each row is its own observation
```
...
The days are each their own column but they are observations.
...
qpcr_untidy
#makes sense in terms of reading and presentation but not analysis
```
