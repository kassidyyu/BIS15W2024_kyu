...
#no x-value of year, spread out across
```
...
```r
expenditures %>% 
  pivot_longer(-expenditure, # don't move
...
```r
# 2 observations in the same cell - name and sex
```
...
```r
#2 untidy data management in one chunk
...
```r
heartrate3 %>% 
  unite(patient_sex, "patient", "sex", sep=" ") #put back together, not a common thing to do but can be done
```
...
```r
tb_data %>% 
  pivot_wider(names_from = "key", #the observations under key will become new columns - make 'cases' one variable and 'population' one variable
              values_from = "value") #the values under value will be moved to the new columns - associate automatically
```
...
```r
#clear x and clear y, long to wide
```


```r
tb_data %>%
  filter(key=="population") #does not hold true to tidy data convention
```
...
```r
# more than one variable represented in each column
```
...
```r
#more readable for presenting the data
```
...
```r
beachbugs_relong <- beachbugs_wide %>%
  pivot_longer(-year, # remember to keep year still
               names_to = "site",
               values_to = "buglevels")
beachbugs_relong
```
