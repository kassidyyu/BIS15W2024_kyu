...
```r
names_list <- strsplit(names, split = " .csv") # just the names, not .csv
names_list
#> [[1]]
#> [1] "Alameda"
#> 
...
```r
names_vec <- unlist(names_list) # vector of names
names_vec
...
names(spider_list) #spider list names are cleaned up
#>  [1] "Alameda"        "Amador"         "Butte"          "Calaveras"     
#>  [5] "Contra Costa"   "Del Norte"      "El Dorado"      "Humboldt"      
#>  [9] "Kern"           "Marin"          "Mariposa"       "Mendocino"     
#> [13] "Monterey"       "Napa"           "Placer"         "Plumas"        
...
```r
spiders_all <- bind_rows(spider_list) # stick all of 32 separate files into 1
spiders_all
#> # A tibble: 270 × 9
#>    Accession Family    Genus   Country State     County Locality Date  Collector
#>        <dbl> <chr>     <chr>   <chr>   <chr>     <chr>  <chr>    <chr> <chr>    
...
inner_exampleDF #only where all columns are present (missing data for 3 and 4, so dropped)
#> # A tibble: 2 × 8
...
left_exampleDF #customer 4 gone, wasn't in the original 'left' data (table_A)
#> # A tibble: 3 × 8
#>   customer_ID n_orders total_value_orders ave_value_orders last_order_date   age
#>         <dbl>    <dbl>              <dbl>            <dbl> <chr>           <dbl>
#> 1           1        5                500              100 12/7/20            50
...
full_exampleDF #puts everything together
#> # A tibble: 4 × 8
#>   customer_ID n_orders total_value_orders ave_value_orders last_order_date   age
#>         <dbl>    <dbl>              <dbl>            <dbl> <chr>           <dbl>
#> 1           1        5                500              100 12/7/20            50
...
```r
spiders_with_locs <- left_join(spiders_all, spiders_locs, by="Accession") #spider locations to spiders all
...
```r
dmy(spiders_with_locs$Date) # if not exactly correct, replace and put NA - check date convention
