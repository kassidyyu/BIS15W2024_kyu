...
```{r}
dim(hbirds) #column = variable, observations in a cell
```
...
```{r}
#Ledford: all variables in all lowercase, underscore - R is sensitive
hbirds <- data.frame(sex=Sex, length=Length, weight_g=Weight) #new name = old name, overwrite old data frame
hbirds
```
...
hsprings <- cbind(hsprings, depth_ft) #cbind to add a new column
hsprings
```
