...
```{r}
hot_springs$scientist <- as.factor(hot_springs$scientist) #replace scientist in the hot springs data frame with the same data as a factor
#factor is repeated sets of type, discrete categories
```

```{r}
levels(hot_springs$scientist) #tells how many different factors there are in a variable
levels(as.factor(hot_springs$spring))
```
...
```{r}
#table(fish) Error - >= 2^31
table(fish$lakeid) #states, number of observations present for each state
```
